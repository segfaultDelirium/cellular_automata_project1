...
function displayPercentageOfOnes() {
  ...
  const element: HTMLElement = document.querySelector("#percentage-of-ones")!;
  element.innerText = `percentage of ones: ${percentage}`
}

function displayCurrentStepCounter() {
  const stepCounter: HTMLElement = document.querySelector("#current-step-counter")!;
...
function makeIndexValid(index: number) {
  const res1 = index < 0 ? index + cellCount : index;
  const res2 = res1 >= cellCount ? res1 - cellCount : res1;
  return res2;
}

function convertToZeroOne(bool: boolean) {
  return bool ? 1 : 0
}

function generateNewRow(currentRow: boolean[]) {
  const newRow = currentRow.map((cell, i) => {
    ...
    const surrounding: number = surr0 * 16 + surr1 * 8 + surr2 * 4 + surr3 * 2 + surr4;
...
function evolveCell(rule: string, surrounding: number) {
...
function displayRule() {
  const ruleElement: HTMLElement = document.querySelector('#rule')!;
  ruleElement.innerText = "rule: " + rule;
}

function updateRule() {
  const newRuleInput: HTMLInputElement = document.querySelector('#new-rule')!;
  const inputValue = newRuleInput!.value
  console.log(newRuleInput);
  console.log(inputValue)

  const errorElement: HTMLElement = document.querySelector('#error-message')!;
